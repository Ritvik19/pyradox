...
class DenselyConnectedNetwork(layers.Layer):
    ...
    """
    def __init__(self, layer_config, batch_normalization=False, dropout=0, activation='relu', **kwargs):
        super().__init__()
    ...
    def __call__(self, inputs):
        ...
        for units in self.layer_config:
            x = DenselyConnected(units, batch_normalization=self.batch_normalization, dropout=self.dropout, activation=self.activation, **self.kwargs)(x)
        return x
...
class DenselyConnectedResnet(layers.Layer):
    ...
    Args:
        ...
        kwargs    (keyword arguments): the arguments for Dense Layer
    """
    def __init__(self, layer_config, batch_normalization=True, dropout=0, activation='relu', **kwargs):
        super().__init__()
        self.layer_config = layer_config
        self.batch_normalization = batch_normalization
        self.dropout = dropout
    ...
    def __call__(self, inputs):
        x = inputs
        for units in self.layer_config:
            x = DenseSkipConnection(units, batch_normalization=self.batch_normalization, dropout=self.dropout, activation=self.activation, **self.kwargs)(x)
        return x